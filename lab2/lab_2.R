...
standardDeviation = function(values) {
  ...
  for (val in values) {
    sum = sum + ((meanValues - mean) ^ 2)
  }
  
  sqrt(1 / (length(values) - 1) * sum)
}
...
plot(liczba_bledow)
table(liczba_bledow)
plot(table(liczba_bledow))
barplot(liczba_bledow)
barplot(table(liczba_bledow))

prop.table(liczba_bledow)
barplot(prop.table(table(liczba_bledow)))

pie(prop.table(table(liczba_bledow)))
